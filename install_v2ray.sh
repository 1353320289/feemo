...
getData() {
    if [[ "$TLS" = "true" || "$XTLS" = "true" ]]; then
        echo ""
        echo -e "# ${GREEN}Feemo--VPS 协议安装运行，运行之前请确认如下条件已经具备：{PLAIN}                #"
        colorEcho ${YELLOW} "  1. 一个伪装域名"
        colorEcho ${YELLOW} "  2. 伪装域名DNS解析指向当前服务器ip（${IP}）"
        
        echo " "
        read -p " 确认满足请按y哦，按其他的默认退出Feemo--VPS：" answer
        if [[ "${answer,,}" != "y" ]]; then
            exit 0
        fi
    ...
    if [[ "$TLS" = "true" || "$XTLS" = "true" ]]; then
        ...
        echo ""
        colorEcho $BLUE "  是否允许流入互联网获取网站？[默认：不允许]"
        echo "    y)允许，可能会造成内存占用，但会有其他ip访问"
        echo "    n)不允许，保障服务器基础运行"
...
module_hotfixes=true' > /etc/yum.repos.d/nginx.repo
        ...
        if [[ "$?" != "0" ]]; then
            colorEcho $RED " 您安装了宝塔，请在宝塔后台安装nginx后再运行Feemo--VPS协议安装"
            exit 1
        fi
    fi
}
...
getCert() {
    ...
    if [[ -z ${CERT_FILE+x} ]]; then
        ...
        if [[ "${res}" != "" ]]; then
            colorEcho ${RED}  " 其他进程占用了80或443端口，请先关闭再运行Feemo--VPS协议安装"
            echo " 端口占用信息如下："
            echo ${res}
            exit 1
        fi
        ...
        [[ -f ~/.acme.sh/${DOMAIN}_ecc/ca.cer ]] || {
            colorEcho $RED " 获取证书失败，该服务器系统可能存在不兼容，需要更换系统"
            exit 1
        }
        KEY_FILE="/etc/v2ray/${DOMAIN}.key"
		CERT_FILE="/etc/v2ray/${DOMAIN}.pem"
...
menu() {
    ...
    echo "#############################################################"
     ...
     echo " -------------"
    echo -e "  ${GREEN}2.${PLAIN}   安装V2ray-${RED}VMESS+mKCP${PLAIN}"
    echo " -------------"
    echo -e "  ${GREEN}3.${PLAIN}   安装V2ray-${RED}VMESS+TCP+TLS${PLAIN}"
    echo " -------------"
    echo -e "  ${GREEN}4.${PLAIN}   安装V2ray-${RED}VMESS+WS+TLS${PLAIN}"
    echo " -------------"
    echo -e "  ${GREEN}5.${PLAIN}   安装V2ray-${RED}VLESS+mKCP${PLAIN}"
    echo " -------------"
    echo -e "  ${GREEN}6.${PLAIN}   安装V2ray-${RED}VLESS+TCP+TLS${PLAIN}"
    echo " -------------"
    echo -e "  ${GREEN}7.${PLAIN}   安装V2ray-${RED}VLESS+WS+TLS${PLAIN}"
    echo " -------------"
    echo -e "  ${GREEN}8.${PLAIN}   安装V2ray-${RED}VLESS+TCP+XTLS${PLAIN}"
    echo " -------------"
    echo -e "  ${GREEN}9.${PLAIN}   安装${RED}trojan${PLAIN}"
    echo " -------------"
    echo -e "  ${GREEN}10.${PLAIN}  安装${RED}trojan+XTLS${PLAIN}"
    echo " -------------"
    echo -e "  ${GREEN}11.${RED}  更新V2ray${PLAIN}"
    ...
    statusText
    echo 
    echo -e "# ${GREEN}注意: 仅供学习交流 严禁用于商业用途--Feemo${PLAIN}                #"      
